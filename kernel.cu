...
__global__ void Test_Kernel(int numSms, int kernelID)
{
...
int main(void)
{
    ...
    smCounts[0] = 4; smCounts[1] = 3;
    smCounts[2] = 1; smCounts[3] = 2;    
    ...
    for(step= 0; step < CONTEXT_POOL_SIZE; step++)
        mythread[step] = std::thread([=]()
        {
            ...
            if(numSms != smCounts[step]){
                cout<< "Context parititioning SM error!\tPlan: " << smCounts[step] << "\tactual: " << numSms <<endl;
            }
            //printf("numSms:%d\n",numSms);
            ...
            Test_Kernel<<<dimGrid,dimBlock>>>(numSms,step);
        });
