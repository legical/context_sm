...
void parametric_measure_global(int N, int iterations, int stride) {
	...
	cudaThreadSynchronize ();
        ...
        cudaMemcpy((void *)h_timeinfo, (void *)duration, sizeof(unsigned int)*4096, cudaMemcpyDeviceToHost);
        cudaMemcpy((void *)h_index, (void *)d_index, sizeof(unsigned int)*4096, cudaMemcpyDeviceToHost);
