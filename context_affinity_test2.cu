...
__global__ void Test_Kernel(int numBlocks, int numSms, int kernelID,
    ...
    // if(blockid % 1 == 0){
    //     //printf("Kernel need %d
    //     sms***\tSMID:%d,\tBlockID:%d,\tThreadID:%d,\tS_time:%f,\tE_time:%f\n",numSms,
    //     smid, blockid, threadid, Start_time, End_time);
    //     //printf("Kernel %d uses %d sms. \tBlock %d is in SM %d\n",kernelID,
    //     numSms, blockid, smid);
    //     //cout << "KernelID\t"<< kernelID << "\tSMnum\t" << numSms <<
    //     "\tBlocknum\t" << numBlocks << "\tBlockID\t" << blockid <<"\tSMID\t" <<
    //     smid << "\tStart_time\t" << Start_time << "\tEnd_time\t" << End_time <<
    //     endl; for(int i =0; i<kernelID;i++)
    //         cout << "\t";
    //     cout << "BlockID\t" << blockid <<"\tSMID\t" << smid << "\tStart_time\t"
    //     << Start_time << "\tEnd_time\t" << End_time << endl;
    // }
...
//     //获取当前设备的 COMPUTE_MODE
//     CUresult err1;
//     err1 = cuDeviceGetAttribute(&pi, CU_DEVICE_ATTRIBUTE_COMPUTE_MODE,
//     device); if (err1 != CUDA_SUCCESS)
//     {
//         printf("cuDeviceGetAttribute Error:%s\n", MyGetdeviceError(err1));
//     }
//     // printf("cuDeviceGetAttribute:%d\n",pi);
...
//         CUresult err2;
//         // Returns the execution affinity setting for the current context
//         err2 = cuCtxGetExecAffinity(&affinity,
//         CU_EXEC_AFFINITY_TYPE_SM_COUNT); if (err2 != CUDA_SUCCESS)
//         {
//             printf("thread cuCtxGetExecAffinity Error:%s\n",
//             MyGetdeviceError(err2));
//         }
...
//         error1 =
//         cudaOccupancyMaxActiveBlocksPerMultiprocessor(&numBlocksPerSm,
//         MyKernel, numThreads, 0); if (error1 != cudaSuccess)
//         {
//             printf("thread cudaOccupancyMaxActiveBlocksPerMultiprocessor
//             Error:%s\n", MyGetRuntimeError(error1));
//         }
...
//         dim3 dimBlock(numThreads, 1, 1);//每个Block中thread数目：numThreads
//         dim3 dimGrid(numBlocks, 1, 1);//每个Grid中Block数目
//         // void *kernelArgs[] = {(int *)&numSms, (int *)&numBlocks, (int
//         *)&clockRate}; /* add kernel args */
//         // // Launches a device function where thread blocks can cooperate
...
//         // cudaError_t error2;
//         // error2 = cudaLaunchCooperativeKernel((void *)MyKernel, dimGrid,
//         dimBlock, kernelArgs);
