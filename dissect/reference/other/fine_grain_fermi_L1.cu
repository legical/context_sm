...
void parametric_measure_global(int N, int iterations, int stride) {
	...
	cudaThreadSynchronize ();
        ...
        error_id = cudaMemcpy((void *)h_timeinfo, (void *)duration, sizeof(unsigned int)*512, cudaMemcpyDeviceToHost);
	if (error_id != cudaSuccess) {
		printf("Error 2.0 is %s\n", cudaGetErrorString(error_id));
	}
        error_id = cudaMemcpy((void *)h_index, (void *)d_index, sizeof(unsigned int)*512, cudaMemcpyDeviceToHost);
	if (error_id != cudaSuccess) {
